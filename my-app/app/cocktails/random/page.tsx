...
async function Random() {
  ...
  const reroll = await fetchRandomCocktail();
  const {name, ingredients, preparation, img} = data[0];
  return (
    ...
    <Link href={`/cocktails/random/${reroll[0]._id}`}>
        <span className="absolute bg-black text-white pl-1 pr-1 right-2 rounded">♻︎ ☈e-☈oll</span>
    </Link>
    ...
    <div className="flex mb-4">
      <div className="w-1/2">
        ...
        <p className="mr-2">
          {preparation.split('.').filter(x => x !== '').map((steps, idx) => <li key={idx} className="list-decimal">{steps}</li>)}
        </p>
      </div>
